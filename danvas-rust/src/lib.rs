...
impl Client {
    ...
    pub fn set_value(&self, id: &str, value: Value) {
        {
            ...
            // Merge (not replace): other accumulated data_patch keys must
            // survive for replay.
            let dp = e.entry("data_patch".to_string())
                      .or_insert_with(|| json!({}));
            if let Some(o) = dp.as_object_mut() {
    ...
    /// The merged live view of one data field — registered data plus
    /// everything streamed since (own panels: the replay accumulator; a
    /// peer's: the hub mirror). The read half behind the generated typed
    /// handles (see `live_handles.rs`, codegened from the contracts).
    pub fn live_get(&self, id: &str, key: &str) -> Option<Value> {
        let st = self.inner.state.lock().unwrap();
        let (data_str, acc): (Option<String>, Option<&Map<String, Value>>) =
            if let Some(reg) = st.registers.get(id) {
                (reg["props"]["data"].as_str().map(String::from),
                 st.updates.get(id))
            } else if let Some(e) = st.panels.get(id) {
                (e.props.get("data").and_then(Value::as_str).map(String::from),
                 Some(&e.state))
            } else {
                (None, None)
            };
        let mut out = data_str
            .and_then(|d| serde_json::from_str::<Value>(&d).ok())
            .and_then(|v| v.get(key).cloned());
        if let Some(a) = acc {
            if let Some(v) = a.get("data_patch")
                              .and_then(Value::as_object)
                              .and_then(|dp| dp.get(key)) {
                out = Some(v.clone());
            }
            if key == "value" {
                if let Some(v) = a.get("post") { out = Some(v.clone()); }
            }
        }
        out
    }

    /// Write one live data field — the write half behind the generated typed
    /// handles. Ownership-aware: on a panel this source owns, `value` rides
    /// the post+data_patch pairing ([`set_value`](Self::set_value)) and other
    /// fields a merged `data_patch`; on a PEER's panel the write routes via
    /// `set_props`, so the owner applies it with its real setters.
    pub fn live_set(&self, id: &str, key: &str, value: Value) {
        let own = self.inner.state.lock().unwrap().registers.contains_key(id);
        if !own {
            self.set_props(id, [(key.to_string(), value)]);
            return;
        }
        if key == "value" {
    ...
    fn handle(&self, msg: &Value) {
        ...
        match kind {
            ...
            "input" => {
                ...
                // A control's committed value is state: fold it into the
                // replay accumulator so live_get()/the typed handles read the
                // browser's latest, and a reconnect replays it (the Rust
                // twin of Python's state_payload).
                if let Some(v) = payload.get("value") {
                    let mut st = self.inner.state.lock().unwrap();
                    if st.registers.contains_key(&id) {
                        st.updates.entry(id.clone()).or_default()
                            .insert("post".into(), v.clone());
                    }
                }
...
// Typed live handles ({Kind}Handle + Client::{kind}_handle), generated by
// build.rs from each template contract's `live` list — see build.rs.
include!(concat!(env!("OUT_DIR"), "/live_handles.rs"));
