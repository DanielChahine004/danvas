...
- **distributions toggle** reveals a per-column mini-chart under each header — a
  ...
  computed in Python and drawn as inline SVG.
...
(function(){
  ...
  if(filter){
    filter.addEventListener('input', function(){
      var q = filter.value.toLowerCase(), vis=0;
...
def _table_body(cols, rows):
    ...
    head_cells = "".join(
        f'<th data-num="{1 if numeric[i] else 0}">{_cell(c)}'
        f'<span class="pc-arrow"></span></th>'
        for i, c in enumerate(cols)
    )
    dist_cells = "".join(
        f"<th>{_distribution([r[i] for r in rows if i < len(r)], numeric[i])}</th>"
        for i in range(ncol)
    )
...
def _distribution(values, numeric):
    ...
    Returns an empty string on any failure so a single odd column never breaks
    the whole table.
    """
    try:
        if numeric:
...
def _bars_svg(heights, titles, width=120, height=28):
...
def _numeric_hist(values, bins=12):
    ...
    if hi == lo:
        svg = _bars_svg([1.0], [f"{lo:g} × {len(nums)}"])
    ...
    return _bars_svg(heights, titles) + cap
...
def _category_bars(values, top=8):
    ...
    titles = [f"{name}: {c}" for name, c in common]
