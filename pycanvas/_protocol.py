...
(``BINARY_*`` in :mod:`pycanvas.bridge`, ``BIN_*`` in ``bridge.js``; the flag
wire keys in :mod:`pycanvas._flags` vs the destructured params in
...
3.  :mod:`pycanvas.bridge` and :mod:`pycanvas._flags` import the codes/keys from
...
# in step with :data:`pycanvas._flags.LAYOUT_FLAGS` (whose ``.wire`` is the same
